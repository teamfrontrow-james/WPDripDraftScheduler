...
class DDS_Bulk_Actions {
	...
	public function process_bulk_action() {
		...
		$primary_action   = isset( $_REQUEST['action'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) : '';
		$secondary_action = isset( $_REQUEST['action2'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['action2'] ) ) : '';
		$action           = '-1' !== $primary_action ? $primary_action : ( '-1' !== $secondary_action ? $secondary_action : '' );

		if ( 'dds_auto_schedule' !== $action ) {
			return;
		}
	...
	public function show_admin_notices() {
		...
		if ( 0 < $scheduled_count ) {
			...
			if ( $start_formatted && $end_formatted ) {
				$message .= ' ' . sprintf(
					/* translators: 1: first scheduled date, 2: last scheduled date. */
					__( 'from %1$s to %2$s.', 'draft-drip-scheduler' ),
					$start_formatted,
					$end_formatted
				);
			}
		...
		if ( 'future' === $post->post_status ) {
			...
			if ( $scheduled_timestamp > current_time( 'timestamp' ) ) {
				$relative_time = sprintf(
					/* translators: %s: relative time until scheduled publication. */
					__( 'In %s', 'draft-drip-scheduler' ),
					$time_diff
				);
				echo '<br><span style="color: #2271b1;">' . esc_html( $relative_time ) . '</span>';
			} else {
				$relative_time = sprintf(
					/* translators: %s: relative time since scheduled publication. */
					__( '%s ago', 'draft-drip-scheduler' ),
					$time_diff
				);
				echo '<br><span style="color: #d63638;">' . esc_html( $relative_time ) . '</span>';
			}
		} else {
	...
	public function handle_scheduled_column_sorting( $query ) {
		...
		if ( 'post_date' === $orderby && isset( $_GET['post_type'] ) ) {
