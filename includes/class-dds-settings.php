...
class DDS_Settings {
	...
	public function render_skip_weekends_field() {
		...
		<label>
			<input type="checkbox" 
					...
					value="1" 
					<?php checked( ! empty( $settings['skip_weekends'] ) ); ?> />
				...
				<table class="form-table" role="presentation">
					<tbody>
						...
						<tr>
							...
							<td><strong><?php echo esc_html( date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $next_safe_timestamp ) ); ?></strong></td>
						</tr>
					</tbody>
				</table>
					...
					<form method="post" action="" style="margin-top: 20px;">
										...
										<span style="color: #666; margin-left: 10px;">
											...
											printf(
												esc_html(
													...
													_n( '(%d draft)', '(%d drafts)', $data['count'], 'draft-drip-scheduler' )
												),
												esc_html( $data['count'] )
											);
											?>
						...
						<p style="margin-top: 20px;">
							<strong>
								...
								printf(
									esc_html(
										/* translators: %d: total number of drafts available. */
										_n( 'Total drafts available: %d', 'Total drafts available: %d', $total_drafts, 'draft-drip-scheduler' )
									),
									esc_html( $total_drafts )
								);
								?>
							</strong>
						</p>
	...
	public function handle_schedule_now() {
		...
		$action = isset( $_POST['dds_action'] ) ? sanitize_text_field( wp_unslash( $_POST['dds_action'] ) ) : '';
		if ( 'schedule_now' !== $action ) {
			return;
		}

		// Check permissions
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to perform this action.', 'draft-drip-scheduler' ) );
		}

		// Verify nonce
		$nonce = isset( $_POST['dds_schedule_now_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['dds_schedule_now_nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'dds_schedule_now' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'draft-drip-scheduler' ) );
		}

		// Get selected post types
		$selected_post_types = isset( $_POST['dds_selected_post_types'] ) && is_array( $_POST['dds_selected_post_types'] )
			? array_map( 'sanitize_text_field', wp_unslash( $_POST['dds_selected_post_types'] ) )
			: array();

		// Validate selected post types
		if ( empty( $selected_post_types ) ) {
			wp_die( esc_html__( 'Please select at least one post type to schedule.', 'draft-drip-scheduler' ) );
		}
		...
		if ( empty( $post_types ) ) {
			wp_die( esc_html__( 'Invalid post types selected.', 'draft-drip-scheduler' ) );
		}
	...
	public function show_schedule_now_notices() {
		...
		if ( ! $screen || 'settings_page_draft-drip-scheduler' !== $screen->id ) {
			return;
		}
		...
		if ( 0 < $total_scheduled ) {
			...
			if ( ! empty( $results_by_type ) ) {
				$message .= '<br><br><strong>' . esc_html__( 'Details:', 'draft-drip-scheduler' ) . '</strong><ul style="margin-left: 20px;">';
		...
		if ( 0 < $total_failed ) {
			$error_message = sprintf(
				/* translators: %d: number of posts that could not be scheduled. */
				_n(
